...
function ProductAttribute({
    ...
    // 編輯屬性時的formData
    const {index, name, alias, stock, placeholder} = formData
    ...
    const editOneAttribute = (alias, idx) => {
        ...
        const item = attrs.filter(item => item.alias === alias)
        const  value = (item.length > 0 && idx >= 0) ? item[0].rows[idx].name : ''
        const  stock = (item.length > 0 && idx >= 0) ? item[0].rows[idx].stock : ''
        setOneAttribute({
            alias: alias,
            index: idx,
            value: value,
                ...
                <div key={attribute.alias} className="flex flex-col max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-900 dark:border-gray-700">
                    ...
                    <div>
                        <ul className='flex flex-wrap gap-3 mt-4'>
                            {attribute && attribute.rows && Array.isArray(attribute.rows) && attribute.rows.map((row, idx1) => (
