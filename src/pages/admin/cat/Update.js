...
import { getOneAPI, postUpdateAPI } from '../../../context/cat/CatAction'
...
import {
    CATNAMEBLANK,
    ALIASBLANK,
    GetCatNameBlankError,
    GetAliasBlankError,
} from '../../../errors/CatError'
...
function UpdateCat() {
    ...
    const initBreadcrumb = [
        ...
        { name: '分類列表', href: '/admin/cat', current: false },
    ]
    const [breadcrumbs, setBreadcrumbs] = useState(initBreadcrumb)
    const [formData, setFormData] = useState({});

    const {name, eng_name, status} = formData
    ...
    const errorReducer = (state=initalError, action) => {
        ...
        switch (action.type) {
            case CATNAMEBLANK:
                nameState = {code: CATNAMEBLANK, message: GetCatNameBlankError().msg}
    ...
    useEffect(() => {
        ...
        const getOne = async (token, scenario) => {
            ...
            setBreadcrumbs(() => {
                return [...initBreadcrumb, { name: data.name, href: '/admon/cat/update', current: true }]
            })
            setFormData((prev) => {
                return {...prev, ...data}
            })
        ...
        if (token !== undefined && token.length > 0) {
            ...
            getOne('', 'create');
            setBreadcrumbs((prev) => {
                return [...prev, { name: '新增品牌', href: '/admon/cat/update', current: true }]
            })
        }
    }, []);
    ...
    const onSubmit = async (e) => {
        ...
        if (name === undefined || name.length === 0) {
			dispatch({type: CATNAMEBLANK})
			isPass = false
        }
        if (eng_name === undefined || eng_name.length === 0) {
        ...
        if (token !== undefined && token !== null && token.length > 0) {
            postFormData.append("cat_token", token)
        }
