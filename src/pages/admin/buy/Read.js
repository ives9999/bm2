...
const ReadBuy = () => {
    ...
    const getData = async (accessToken, page, perpage, _params) => {
        const data = await getReadAPI('buy', page, perpage, params, accessToken);
    ...
    const onDragEnd = async (e) => {
        ...
        if (active !== over) {
            ...
            console.info(res);
            setIsLoading(true);
            const data = await postUpdateSortOrderAPI('buy', JSON.stringify(res), auth.accessToken);
    ...
    if (!isGetComplete) {
        ...
        return (
            <div className='p-4'>
                ...
                <h2 className='text-MyWhite text-3xl mb-4'>進貨單列表</h2>
